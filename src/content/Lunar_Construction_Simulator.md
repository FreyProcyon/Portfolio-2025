...
**月球建造模拟（Lunar Construction Simulator）** 是一个运行在 **Meta Quest 3** 上的沉浸式 VR 建模工具，灵感来自 Rhino/CAD 的建模流程，面向未来 **月球工程与远程建造** 场景。:contentReference[oaicite:0]{index=0}  
...
- **系统架构与开发**
  ...
  - 实现创建、选择、移动、旋转、缩放、删除与材质切换等完整建模流程:contentReference[oaicite:2]{index=2}  
...
**场景设定：**  
> “想象你是一名远程操作工程师，通过 VR 接口控制月球上的 3D 打印车，为基地建造墙体与支撑结构。” :contentReference[oaicite:5]{index=5}  
...
4. **材质切换（Change Material）**  
   ...
   - 让用户在视觉上区分“结构件 / 外壳 / 连接件”，而不只是换颜色:contentReference[oaicite:8]{index=8}  
...
- **Teleport 移动（Teleport Locomotion）**  
  - 使用官方 Teleport 模块实现空间移动，支持在场景中来回浏览与接近结构:contentReference[oaicite:9]{index=9}  
...
- **参与者（Participants）：** N=10，包含无建模经验、入门与高级用户（40% / 20% / 40%）:contentReference[oaicite:10]{index=10}  
- **方法（Method）：**  
  - Task-based + Think-aloud（任务驱动 + 思维口述）  
  - 使用屏幕录制记录手部/控制器操作与 UI 状态  
- **任务（Tasks）：**:contentReference[oaicite:11]{index=11}  
...
- “我可以完成给定任务”主观评分平均约 **4.2/5**（n=10）:contentReference[oaicite:12]{index=12}  
- 预览与网格吸附的“有帮助程度”平均约 **4.3/5**（n=10）  
- Teleport 易用性平均 **3.9/5**：多数人觉得舒适，但表示对“高层结构”观察略吃力:contentReference[oaicite:13]{index=13}  
- 材质切换易用性在尝试过的用户中为 **5/5**（n=3），说明材质被理解为“材质性质变化”，而非仅颜色变化（样本较小，有待扩大）  

### 质性发现（Qualitative Insights）

代表性反馈包括：:contentReference[oaicite:15]{index=15}  
...
> 🎥 *此处预留：在网页中嵌入 B 站演示视频 `<https://www.bilibili.com/video/BV12cUCBHEha>` 的 iframe 或自定义播放器。*
