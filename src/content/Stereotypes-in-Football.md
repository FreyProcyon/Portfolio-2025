...
设计了一套 **4 步交互式数据故事**，帮助读者从“刻板印象”回到可验证的数据事实。:contentReference[oaicite:0]{index=0}  
...
每一页顶部都用一句 **主题句（topic sentence）** 建立阅读上下文，版面基于 12 列网格与留白控制视觉负荷，符合 Gestalt 的“简洁律（Prägnanz）”。:contentReference[oaicite:1]{index=1}  
...
- 独立完成 **数据获取与清洗**：  
  - FIFA 世界排名、Kaggle 球员能力数据、SalarySport 薪资数据:contentReference[oaicite:2]{index=2}  
- 设计整体 **数据故事结构与页面叙事**（信息架构 + 交互顺序）:contentReference[oaicite:3]{index=3}  
- 在 Flourish 中实现四个交互图表及 Story 串联  
- 根据 Tufte、Munzner 等可视化原则优化视觉编码、色彩与交互模式:contentReference[oaicite:4]{index=4}  
- 撰写设计说明文档与注释海报，对每个图表的设计决策进行反思与论证:contentReference[oaicite:5]{index=5}  

---

## 📚 数据与问题定义（Data & Question）

**数据来源：**:contentReference[oaicite:6]{index=6}  

1. FIFA 男足世界排名  
2. Kaggle “Football Players Data” – 球员基础信息与能力评分  
3. Salary Sport – 五大联赛及其他主流联赛球员周薪  
4. 课程讲义与教材：Macdonald & Cordeil（信息可视化）、Munzner、Tufte 等:contentReference[oaicite:7]{index=7}  
...
色彩使用单调递增的 **Sequential Blue Scale**，固定映射到完整数值区间，既减少截断坐标轴和 Lie Factor 带来的误导风险，又符合 **色盲友好（Colour-blind Safe）** 的建议。:contentReference[oaicite:11]{index=11}  
...
交互上提供 Position / Nationality / Salary Range 过滤器，切换时触发平滑动画（Animated Transition），维持 **对象恒常性（Object Constancy）**，减轻短期记忆负担。:contentReference[oaicite:13]{index=13}  
...
该设计有意避免 3D 柱状图等装饰性编码，遵循 Munzner 的 “**No unjustified 3D**” 原则。:contentReference[oaicite:15]{index=15}  
