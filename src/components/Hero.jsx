      ...
      <div className="heroMeta left">
        ...
        <div className="researchWrap">
          ...
          <span className="researchLink">
            AI 驱动的 XR 意图式交互
            <span className="researchHint" aria-hidden>
              {" "}
              ⓘ
            </span>
            <span className="researchTooltip" role="tooltip">
              <div className="ttTitle">
                AI 驱动的 XR 意图式交互
                <span className="ttTitleSub">
                  Intent-Based Interaction in XR
                </span>
              </div>

              <div className="ttBody">
                <p>
                  我目前的研究聚焦于 AI 如何推动 XR
                  交互从“指令式（command-based）”向“意图式（intent-based）”转变。
                  传统 XR
                  系统依赖按钮、菜单与模式切换，用户需要明确告诉系统“怎么做”；而在多模态
                  AI（语音理解、语义推断、上下文感知）加持下， XR
                  有机会转向更自然的交互：用户只需表达“想做什么”，系统自动推断并完成具体操作。
                </p>
                <p>
                  在实践中，我围绕{" "}
                  <b>注视（Gaze）+ 手势（Gesture）+ 语音（Speech）</b>{" "}
                  等多模态输入，构建基于 AI 的 XR 交互 Demo，
                  推断用户在空间环境中的操作意图（选择、移动、缩放、复制等），并在
                  AR / VR 场景中执行对应行为。 系统以 Unity XR 为核心，并结合
                  OpenAI / Google 的多模态 API，以贴近真实产品开发的方式模拟
                  AI-native XR 交互体验。
                </p>
                <p>
                  我同时关注交互效率、认知负荷、可控性与用户信任感，通过对比传统指令式交互与意图式交互，探索
                  AI 介入后 XR 体验的真实变化， 为未来 AI-native XR
                  应用、空间编辑工具与沉浸式交互系统提供可落地的设计参考。
                </p>
              </div>
        ...
        {lanes.map((lane, idx) => (
          ...
          >
            ...
            <div className="skillTrack">
              {skills.map((s) => (
                <span key={`${idx}-${s}-a`} className="skillPill">
                  {s}
                </span>
              ))}
              {skills.map((s) => (
                <span key={`${idx}-${s}-b`} className="skillPill">
                  {s}
                </span>
              ))}
            </div>
